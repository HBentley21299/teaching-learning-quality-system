...
$fileName = if ($Remove) { "004_remove_dashboard_reporting_test_data.sql" } else { "003_seed_dashboard_reporting_test_data.sql" }
$scriptPath = Join-Path $root "database\seed\local\$fileName"

Write-Host ($(if ($Remove) { "Removing" } else { "Creating" })) "local dashboard reporting test data..."
& $sqlCmd -S $Server -d $Database -E -b -i $scriptPath
if ($LASTEXITCODE -ne 0) {
    throw "$sqlCmd failed with exit code $LASTEXITCODE."
}

Write-Host ($(if ($Remove) { "Local dashboard reporting test data removed." } else { "Local dashboard reporting test data is ready." }))
